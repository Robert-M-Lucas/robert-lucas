    ...
    return (
        <ProjWrapper>
            <Pp>
                ...
                <PLink to={getProjectPath(WebsiteProject.name)}>here</PLink>. It
                was created using Django, a templating framework for Python
                (i.e. a framework where the server is responsible for generating
                individual webpages to send to the client). This contrasts
                React's approach (with which this website is created with) where
                the website is a single bundle sent to the client (although some
                parts can be loaded dynamically if too big).
            </Pp>

            <Pp>
                Django's approach creates a large amount of friction with
                dynamic content as you are presented with the choice of either
                statically rendering data into a template on the server, then
                sending this to the client, or sending the client Javascript to
                load content dynamically. The first option can cause
                first-page-load delays as the server fetches data for the page,
                whereas the latter can be very difficult to implement, as it
                must be done over conventional web requests, with all the
                drawbacks, and hard to integrate libraries of vanilla JS.
            </Pp>
            ...
            <Pp>
                Not only does React code save time in creating dynamic UI, it
                also encourages better design patterns as better abstractions
                are easier to make.
            </Pp>
            ...
            <Pp>
                Finally, the vast number of React-compatible libraries allow the
                easy inclusion of additional functionality (such as the addition
                of the above code blocks) whereas in Django, the extra effort
                required to add these led me to use images instead.
            </Pp>
        </ProjWrapper>
    )
}
