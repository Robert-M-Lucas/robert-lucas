...
function RaytracingEntryPage() {
    ...
    <Pimg image={preview} alt={"TODO"} caption={"Preview with reduced resolution and lighting calculations"}/>
    <Pimg image={render} alt={"TODO"} caption={"Render with full lighting effects. Note that one of the squares at the back is white in colour, but appears slightly red as it is receiving indirect lighting from the red square"}/>
    <Pp>
    ...
    <Pimg image={raytracing} alt={"TODO"} caption={"Raytracing diagram"} source={{name: "Wikimedia", url: "https://commons.wikimedia.org/wiki/File:Ray_trace_diagram.svg"}}/>
    <Pp>
    ...
    <Pimg image={indirect} alt={"TODO"} source={{name: "Scratchapixel", url: "https://www.scratchapixel.com/lessons/3d-basic-rendering/ray-tracing-overview/light-transport-ray-tracing-whitted.html"}}/>
    <Pimg image={indirect2} alt={"TODO"} source={{name: "Scratchapixel", url: "https://www.scratchapixel.com/lessons/3d-basic-rendering/global-illumination-path-tracing/global-illumination-path-tracing-practical-implementation.html"}}/>
    <Pp>
    ...
    <Pimg legacyNaturalWidth image={v3} alt={"TODO"} caption={"Snippet of 3D vector implementation with utility functions"}/>
    <Pimg legacyNaturalWidth image={v3_op} alt={"TODO"} caption={"Implementing operators for 3D vectors for easier use"}/>
    <Pp>
        I'd then use a 3D vector to make a line:
    </Pp>
    <Pimg legacyNaturalWidth image={line} alt={"TODO"} caption={"Snippet of implementation for a line"}/>
    <Pp>
        And then I'd use this in the calculations needed for a plane:
    </Pp>
    <Pimg legacyNaturalWidth image={plane} alt={"TODO"} caption={"Snippet of implementation for a plane"}/>
    <Pimg legacyNaturalWidth image={plane_intersection} alt={"TODO"} caption={"Calculating the intersection between the plane and a line by using matrices and Cramer's rule"}/>
    <Pp>
    ...
    <Pimg legacyNaturalWidth image={finding_object} alt={"TODO"} caption={"Finding the closest object given a ray (line)"}/>
    <Pp>Scene configuration:</Pp>
    <Pimg legacyNaturalWidth image={object_config} alt={"TODO"} caption={"Object configuration"}/>
    <Pimg legacyNaturalWidth image={light_config} alt={"TODO"} caption={"Light configuration"}/>
    <Pimg legacyNaturalWidth image={scene_config} alt={"TODO"} caption={"Scene configuration"}/>
    ...
    <Pimg image={failed3} alt={"TODO"} caption={"I don't even want to know"}/>
