...
function GeometricPatternEntryPage() {
    return (
        <ProjWrapper>
            ...
            />
            <Pp>
                This is a simple project I've made to generate these geometric
                patterns which have all sorts of applications such as website
                backgrounds. It works by generating a grid of squares with lines
                dividing them into triangles (these lines can generate either
                way). The points of these squares are then translated randomly
                to create that triangle pattern
            </Pp>
            <Pimg
                image={pattern2}
                alt={"TODO"}
            ...
            />
            <Pp>
                The colours are generated by finding a point's distance from the
                four corners and mixing the four corners' colours
                proportionately. A corner's colour can be set to 'None' to make
                it have no effect. The random color variance is achieved by
                generating a random shade of grey and adding or subtracting it
                from the triangle's color
            </Pp>
            <Pimg
                legacyNaturalWidth
                image={code}
