  ...
  return (
    <ProjWrapper>
      ...
      <P_p>
        ...
        length in bytes follows by a UID that tells the recipient how to handle
        the packet such as splitting up a piece move packet into int
        move_from_x, int move_from_y, int move_to_x, int move_to_y. The system
        also functions by having one player run a server and a client and the
        other run just a server. This allows the client code to be fully
      ...
      <P_p>
        ...
        For example in the diagram below '8' on the bottom level was found and
        as the third layer is maximising you know that the node above will have
        a value of '8' or higher no matter what. As the layer above that (the
        second layer) is minimising and the adjacent node '4' has been found the
        '8' (or more) node won't be picked no matter what is found so we can
      ...
      <P_p>
        ...
        the game (Unity) lag so I decided against using multithreading at all.
      ...
      <P_p>
        ...
        You can then send a packet which will be converted to bytes and sent to
      ...
      <P_p>
        ...
        recipient should decode and handle the packet. Then there are pairs of
