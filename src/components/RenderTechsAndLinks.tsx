...
export default function RenderTechsAndLinks({ currently_working_on, technologies, links }: Props) {
    if (technologies.length === 0 && (links === undefined || links!.length === 0)) {
        return <>{currently_working_on && <i className="text-muted">Currently working on</i>}</>;
    }

    return <span>
        {currently_working_on && <><i className="text-muted">Currently working on</i> |&nbsp;</>}
